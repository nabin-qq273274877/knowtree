...
- **LLM 配置**：API Base URL、API Key、模型名、temperature（输出长度不限制，由模型上限决定）；提供「测试连接」按钮
...
1. **手动编辑（主通道）**：日常增删改，逐条录入知识点；「➕ 新建 / ＋下级 / ＋同级」快速搭建骨架，随学习进度逐步充实正文内容。
...
```
knowtree（双击或命令行）
    ├─ 默认监听 127.0.0.1:6006（-addr 参数或环境变量可改）
...
- 首次启动自动建库建表并引导初始化，浏览器打开 `http://127.0.0.1:6006` 即进入
...
| 部署 | 主形态：单个可执行文件直接运行（≤ 30MB）；可选 Docker（镜像 ≤ 30MB）；单端口（默认 `6006`，可配）；数据落本地 data 目录或挂载卷 |
...
| M4 LLM | 设置面板、流式讲解、**出题 + 批改闭环** | 讲解/出题/批改能力跑通 |
